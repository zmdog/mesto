...
const renderCards = new Section(".elements__elements-grid", (item, info) => {
...
const popUpModalPlace = new PopupWithForm({
    ...
    handlerSubmit: infoProfile => {
        ...
        api.postCard(infoProfile)
            .then(result => {
                renderCards.addItemPrepend(renderCards.render(result, fetchedInfo))
...
Promise.all([
    ...
    .then(([info, cards]) => {
        ...
        userInformation.renderingInfoProfile(fetchedInfo)
        renderCards.renderItems(cards, fetchedInfo)
    })
    .catch((err) => {
        console.log(err);
    })
